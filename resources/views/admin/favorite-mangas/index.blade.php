...
@section('content')
<div x-data="mangaManager()">
    <div class="mb-8 flex justify-between items-end">
        <div>
            <h1 class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400 drop-shadow-md">Manga Favorite</h1>
            <p class="text-gray-400 mt-1">Mengelola daftar Manga Favorite untuk ditampilkan di halaman utama.</p>
        </div>
        <button @click="openModal('create')" class="bg-[#9D00FF] hover:bg-[#b033ff] text-white px-6 py-3 rounded-xl font-bold shadow-[0_0_15px_rgba(157,0,255,0.4)] hover:shadow-[0_0_25px_rgba(157,0,255,0.6)] transition-all flex items-center gap-2 transform hover:-translate-y-1">
            ...
            <span class="hidden sm:inline">Tambah Manga</span>
        </button>
    </div>
            ...
            <table class="w-full text-left text-gray-300">
                <thead class="bg-black/50 text-xs uppercase font-bold text-gray-500 border-b border-gray-800">
                    <tr>
                        ...
                        <th class="p-4 tracking-wider">Author</th>
                ...
                <tbody class="divide-y divide-gray-800/50">
                    @forelse($favoriteMangas as $manga)
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        <td class="p-4">
                            <img src="{{ Storage::url($manga->cover_image) }}" alt="Cover" class="w-16 h-20 object-cover rounded-md shadow-md border border-gray-700">
                        </td>
                        <td class="p-4 font-bold text-white">{{ $manga->title }}</td>
                        <td class="p-4 text-gray-400">{{ $manga->author ?? '-' }}</td>
                        <td class="p-4 text-gray-400">{{ $manga->release_year ?? '-' }}</td>
                        <td class="p-4 text-cyan-500 font-bold">{{ $manga->status ?? '-' }}</td>
                        <td class="p-4 flex gap-3 justify-end h-full mt-4">
                            <button @click="openModal('edit', {{ $manga->id }}, '{{ addslashes($manga->title) }}', '{{ addslashes($manga->author) }}', '{{ $manga->release_year }}', '{{ addslashes($manga->status) }}')" class="text-yellow-500 hover:text-yellow-400 bg-yellow-500/10 hover:bg-yellow-500/20 p-2 rounded-lg transition-colors border border-transparent hover:border-yellow-500/30" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </button>
                            <form action="{{ route('admin.favorite-mangas.destroy', $manga) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                @csrf
                                @method('DELETE')
        ...
        @forelse($favoriteMangas as $manga)
            <div class="bg-gray-900/60 backdrop-blur-md border border-gray-800 rounded-2xl shadow-lg p-4 relative overflow-hidden group flex flex-col">
                <div class="flex gap-4 mb-4">
                    <img src="{{ Storage::url($manga->cover_image) }}" alt="Cover" class="w-20 h-28 object-cover rounded-lg shadow-md border border-gray-700">
                    <div class="flex-1">
                        <h3 class="font-bold text-lg text-white drop-shadow-sm">{{ $manga->title }}</h3>
                        <p class="text-xs text-[#9D00FF] font-semibold">{{ $manga->author }} • {{ $manga->release_year }}</p>
                        <p class="text-sm text-cyan-400 font-bold mt-1">{{ $manga->status }}</p>
                    </div>
                </div>
                <!-- Actions -->
                <div class="flex gap-2 border-t border-gray-800 pt-4">
                    <button @click="openModal('edit', {{ $manga->id }}, '{{ addslashes($manga->title) }}', '{{ addslashes($manga->author) }}', '{{ $manga->release_year }}', '{{ addslashes($manga->status) }}')" class="flex-1 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 py-2 rounded-xl flex items-center justify-center transition-colors min-h-[44px]">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    </button>
                    <form action="{{ route('admin.favorite-mangas.destroy', $manga) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin?');" class="flex-1 flex">
                        @csrf
                        @method('DELETE')
    ...
    <div x-show="showModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4" x-transition.opacity>
        <div @click.away="closeModal()" class="bg-gray-900 border border-[#9D00FF]/50 rounded-2xl shadow-[0_0_30px_rgba(157,0,255,0.2)] w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col" x-transition.scale>
            <div class="p-6 border-b border-gray-800 bg-black/40 flex-shrink-0">
                <h2 class="text-xl font-bold text-white" x-text="mode === 'create' ? 'Tambah Manga' : 'Edit Manga'"></h2>
            </div>
            <div class="p-6 overflow-y-auto custom-scrollbar flex-1">
                <form :action="formAction" method="POST" enctype="multipart/form-data" id="mangaForm">
                    @csrf
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="author" x-model="formData.author" class="w-full bg-black border border-gray-700 text-white text-base rounded-lg px-4 py-3 focus:outline-none focus:border-[#9D00FF] focus:ring-1 focus:ring-[#9D00FF] transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)]">
                    </div>
                    ...
                    <div class="mb-4 flex gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="text" name="status" x-model="formData.status" class="w-full bg-black border border-gray-700 text-white text-base rounded-lg px-4 py-3 focus:outline-none focus:border-[#9D00FF] focus:ring-1 focus:ring-[#9D00FF] transition-all shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)]" placeholder="e.g. Ongoing">
                        </div>
                    </div>
                </form>
            </div>
            <div class="p-6 border-t border-gray-800 bg-black/40 flex justify-end gap-3 flex-shrink-0">
                ...
                <button type="button" @click="document.getElementById('mangaForm').submit()" class="bg-[#9D00FF] text-white px-5 py-2 rounded-lg font-bold shadow-[0_0_15px_rgba(157,0,255,0.4)] hover:bg-[#b033ff] hover:shadow-[0_0_25px_rgba(157,0,255,0.8)] transition-all">
                    Simpan
                </button>
            </div>
        </div>
...
function mangaManager() {
    return {
        showModal: false,
        mode: 'create',
        formAction: '{{ route('admin.favorite-mangas.store') }}',
        formData: {
            id: null,
            title: '',
            author: '',
        ...
        openModal(mode, id = null, title = '', author = '', release_year = '', status = '') {
            this.mode = mode;
            this.formData.id = id;
            ...
            if (mode === 'edit') {
                this.formAction = `/admin/favorite-mangas/${id}`;
            } else {
                this.formAction = '{{ route('admin.favorite-mangas.store') }}';
            }
